...
# MAGIC %sql
# MAGIC
# MAGIC select distinct(OrderDate) from parquet.`abfss://silver@buyanythingsalesstorage.dfs.core.windows.net/silverdata`
# MAGIC
...
df_src = spark.sql("""
                   select distinct OrderDate from parquet.`abfss://silver@buyanythingsalesstorage.dfs.core.windows.net/silverdata`
                   """)
df_src = df_src.withColumn('Year', year(df_src.OrderDate))
df_src = df_src.withColumn('Month', month(df_src.OrderDate))
df_src = df_src.withColumn('Day', dayofmonth(df_src.OrderDate))
df_src.display()
...
path = 'abfss://gold@buyanythingsalesstorage.dfs.core.windows.net/dim_Date'
if DeltaTable.isDeltaTable(spark, path):
    df_sink=spark.sql("""
                   select dim_date_key, OrderDate, Year, Month, Day from delta.`abfss://gold@buyanythingsalesstorage.dfs.core.windows.net/dim_Date`
                   """)
    ...
    #                     """)
    df_sink=spark.createDataFrame([],schema="dim_date_key int, OrderDate date, Year int, Month int, Day int") #Create a DataFrame with zero rows but with the specified schema
...
if incremental_flag=="0":
    ...
    max_value = df_old.select(max("dim_date_key")).collect()[0][0]
...
# MAGIC %sql
# MAGIC
# MAGIC select * from delta.`abfss://gold@buyanythingsalesstorage.dfs.core.windows.net/dim_Date`
