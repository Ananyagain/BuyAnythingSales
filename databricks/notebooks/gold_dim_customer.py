...
# MAGIC %sql
# MAGIC
# MAGIC select distinct(customerID) as customerID, customerName, country from parquet.`abfss://silver@buyanythingsalesstorage.dfs.core.windows.net/silverdata`
# MAGIC
...
df_src = spark.sql("""
                   select distinct(customerID) as customerID, customerName, country from parquet.`abfss://silver@buyanythingsalesstorage.dfs.core.windows.net/silverdata`
                   """)
df_src.display()
...
path = 'abfss://gold@buyanythingsalesstorage.dfs.core.windows.net/dim_Customer'

if DeltaTable.isDeltaTable(spark,path):
    df_sink=spark.sql("""
                   select dim_cust_key, CustomerID, CustomerName, Country from delta.`abfss://gold@buyanythingsalesstorage.dfs.core.windows.net/dim_Customer`
                   """)
    ...
    #                     """)
    df_sink=spark.createDataFrame([],schema="dim_cust_key int, CustomerID int, CustomerName string,  Country string") #Create a DataFrame with zero rows but with the specified schema
...
df = df_src.join(df_sink, df_src.customerID == df_sink.CustomerID, "left").select(df_src.customerID, df_src.customerName,  df_src.country, df_sink.dim_cust_key)
df.display()
...
if incremental_flag=="0":
    ...
    max_value = df_old.select(max("dim_cust_key")).collect()[0][0]
...
df = df.withColumn("dim_cust_key", max_value + monotonically_increasing_id())
df.display()
...
path = 'abfss://gold@buyanythingsalesstorage.dfs.core.windows.net/dim_Customer'
#storing delta tables in adls, hence need to check: if table exists then update else create new
...
# MAGIC %sql
# MAGIC
# MAGIC select * from delta.`abfss://gold@buyanythingsalesstorage.dfs.core.windows.net/dim_Customer`
