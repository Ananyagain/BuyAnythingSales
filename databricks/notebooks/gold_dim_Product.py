...
# MAGIC %sql
# MAGIC
# MAGIC select distinct ProductID, ProductCategory, ProductName, UnitPrice from parquet.`abfss://silver@buyanythingsalesstorage.dfs.core.windows.net/silverdata` 
# MAGIC
...
df_src = spark.sql("""
                   select distinct(ProductID) as ProductID, ProductCategory, ProductName, UnitPrice from parquet.`abfss://silver@buyanythingsalesstorage.dfs.core.windows.net/silverdata`
                   """)
df_src.display()
...
path = 'abfss://gold@buyanythingsalesstorage.dfs.core.windows.net/dim_Product'
if DeltaTable.isDeltaTable(spark,path):
    df_sink=spark.sql("""
                   select dim_product_key, ProductID, ProductCategory, ProductName, UnitPrice from delta.`abfss://gold@buyanythingsalesstorage.dfs.core.windows.net/dim_Product`
                   """)
    ...
    #                     """)
    df_sink=spark.createDataFrame([],schema="dim_product_key int, ProductID int, ProductCategory string, ProductName string, UnitPrice decimal(10,2)")
...
if incremental_flag=="0":
    ...
    max_value = df_old.select(max("dim_product_key")).collect()[0][0]
...
# MAGIC %sql
# MAGIC
# MAGIC select * from delta.`abfss://gold@buyanythingsalesstorage.dfs.core.windows.net/dim_Product`
